...
#define PCI_MSR_IA32_TME_ACTIVATE    0x982
...
fu_plugin_add_security_attr_intel_tme_enabled(FuPlugin *plugin, FuSecurityAttrs *attrs)
{
	...
	/* create attr (which will already have been created in the cpu plugin) */
	attr = fu_security_attrs_get_by_appstream_id(attrs, FWUPD_SECURITY_ATTR_ID_ENCRYPTED_RAM);
	if (attr == NULL) {
		g_warning("failed to get attr for %s", FWUPD_SECURITY_ATTR_ID_ENCRYPTED_RAM);
		return;
	}
